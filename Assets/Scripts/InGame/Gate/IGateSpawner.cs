...
namespace InGame
{
    ...
    {
        EnemyEntity[] Spawn(Vector2 gatePosition, EnemyEntity enemyPrefab);
    }
    
    [Serializable]
    public class GateSpawnSingle : IGateSpawner
    {
        public EnemyEntity[] Spawn(Vector2 gatePosition, EnemyEntity enemyPrefab)
        {
            var enemy = EnemyPool.Instance.Get(enemyPrefab, null);
