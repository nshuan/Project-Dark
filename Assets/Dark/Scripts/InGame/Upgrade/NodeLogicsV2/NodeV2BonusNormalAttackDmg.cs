...
{
    [Serializable]
    [ConfigNodeLogicTypeV2(NodeBonusTypeV2.BonusNormalAttackDmg)]
    public class NodeV2BonusNormalAttackDmg : INodeActivateLogicV2, INodeLogicGeneratorV2
    {
        public float[] value;
        public bool isMul;
        ...
        public void ActivateNode(int level, ref UpgradeBonusInfoV2 bonusInfo)
        {
            ...
            if (isMul) bonusInfo.bonusNormalAttack.bonusNormalAttackDmg.mul += value[level - 1];
            else bonusInfo.bonusNormalAttack.bonusNormalAttackDmg.add += value[level - 1];
        }
        ...
        public (string, string) GetBeforeAfterValueTotalStat(int level, ref UpgradeBonusInfoV2 bonusInfo)
        {
            ...
            before = LevelUtilityV2.GetNormalAttackDamage().Item1.ToString(GameConst.FloatFormat, CultureInfo.InvariantCulture);

            if (level > value.Length)
                return (before, before);
 
            var dmgMultiply = bonusInfo.bonusNormalAttack.bonusNormalAttackDmg.mul;
            var dmgPlus = bonusInfo.bonusNormalAttack.bonusNormalAttackDmg.add;

            ActivateNode(level, ref bonusInfo);
            
            var after = "";
            after = LevelUtilityV2.GetNormalAttackDamage().Item1.ToString(GameConst.FloatFormat, CultureInfo.InvariantCulture);
            
            bonusInfo.bonusNormalAttack.bonusNormalAttackDmg.mul = dmgMultiply;
            bonusInfo.bonusNormalAttack.bonusNormalAttackDmg.add = dmgPlus;
