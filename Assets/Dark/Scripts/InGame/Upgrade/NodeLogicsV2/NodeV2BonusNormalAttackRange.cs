...
{
    [Serializable]
    [ConfigNodeLogicTypeV2(NodeBonusTypeV2.BonusNormalAttackRange)]
    public class NodeV2BonusNormalAttackRange : INodeActivateLogicV2, INodeLogicGeneratorV2
    {
        public float[] value;
        public bool isMul;
        ...
        public (string, string) GetBeforeAfterValueTotalStat(int level, ref UpgradeBonusInfoV2 bonusInfo)
        {
            ...
            before = LevelUtilityV2.GetNormalAttackRange(Vector2.right).ToString(GameConst.FloatFormat, CultureInfo.InvariantCulture);

            if (level > value.Length)
                return (before, before);
 
            var rangeMultiply = bonusInfo.bonusNormalAttack.bonusNormalAttackRange.mul;
            var rangePlus = bonusInfo.bonusNormalAttack.bonusNormalAttackRange.add;

            ActivateNode(level, ref bonusInfo);
            
            var after = "";
            after = LevelUtilityV2.GetNormalAttackRange(Vector2.right).ToString(GameConst.FloatFormat, CultureInfo.InvariantCulture);
            
            bonusInfo.bonusNormalAttack.bonusNormalAttackRange.mul = rangeMultiply;
            bonusInfo.bonusNormalAttack.bonusNormalAttackRange.add = rangePlus;
