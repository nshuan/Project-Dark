...
{
    [Serializable]
    [ConfigNodeLogicTypeV2(NodeBonusTypeV2.BonusBaseVestigeCollectSize)]
    public class NodeV2BonusBaseVestigeCollectSize : INodeActivateLogicV2, INodeLogicGeneratorV2
    {
        public float[] value;
        public bool isMul;
        ...
        public void ActivateNode(int level, ref UpgradeBonusInfoV2 bonusInfo)
        {
            ...
            if (isMul) bonusInfo.bonusBase.bonusVestigeCollectSize.mul += value[level - 1];
            else bonusInfo.bonusBase.bonusVestigeCollectSize.add += value[level - 1];
        }
        ...
        public (string, string) GetBeforeAfterValueTotalStat(int level, ref UpgradeBonusInfoV2 bonusInfo)
        {
            ...
            before = LevelUtilityV2.GetVestigeCollectSize().ToString(GameConst.FloatFormat, CultureInfo.InvariantCulture);

            if (level > value.Length)
                return (before, before);
 
            var vestigeCollectSizeMultiply = bonusInfo.bonusBase.bonusVestigeCollectSize.mul;
            var vestigeCollectSizePlus = bonusInfo.bonusBase.bonusVestigeCollectSize.add;
        ...
        public INodeActivateLogicV2 Generate(string subType, List<string> listValue, bool mul)
        {
            ...
            {
                throw new Exception($"Invalid BonusBaseVestigeCollectSize value string: {listValue[0]}");
            }
        }
    }
}