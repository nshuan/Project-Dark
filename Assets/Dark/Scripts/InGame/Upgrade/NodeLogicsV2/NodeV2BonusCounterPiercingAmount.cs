...
{
    [Serializable]
    [ConfigNodeLogicTypeV2(NodeBonusTypeV2.BonusCounterPiercingAmount)]
    public class NodeV2BonusCounterPiercingAmount : INodeActivateLogicV2, INodeLogicGeneratorV2
    {
        public float[] value;
        public bool isMul;
        ...
        public void ActivateNode(int level, ref UpgradeBonusInfoV2 bonusInfo)
        {
            ...
            if (isMul) bonusInfo.bonusCounter.bonusPiercingAmount.mul += value[level - 1];
            else bonusInfo.bonusCounter.bonusPiercingAmount.add += value[level - 1];
        }
        ...
        public (string, string) GetBeforeAfterValueTotalStat(int level, ref UpgradeBonusInfoV2 bonusInfo)
        {
            ...
            var amountMultiply = bonusInfo.bonusCounter.bonusPiercingAmount.mul;
            var amountPlus = bonusInfo.bonusCounter.bonusPiercingAmount.add;
        ...
        public INodeActivateLogicV2 Generate(string subType, List<string> listValue, bool mul)
        {
            ...
            {
                throw new Exception($"Invalid BonusCounterPiercingAmount value string: {listValue[0]}");
            }
        }
    }
}
