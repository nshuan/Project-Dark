...
{
    [Serializable]
    [ConfigNodeLogicTypeV2(NodeBonusTypeV2.BonusMoveDmg)]
    public class NodeV2BonusMoveDmg : INodeActivateLogicV2, INodeLogicGeneratorV2
    {
        public float[] value;
        public bool isMul;
        ...
        public void ActivateNode(int level, ref UpgradeBonusInfoV2 bonusInfo)
        {
            ...
            if (isMul) bonusInfo.bonusMove.bonusMoveDmg.mul += value[level - 1];
            else bonusInfo.bonusMove.bonusMoveDmg.add += value[level - 1];
        }
        ...
        public (string, string) GetBeforeAfterValueTotalStat(int level, ref UpgradeBonusInfoV2 bonusInfo)
        {
            ...
            before = LevelUtilityV2.GetFlashDamage().ToString(GameConst.FloatFormat, CultureInfo.InvariantCulture);

            if (level > value.Length)
                return (before, before);
 
            var dmgMultiply = bonusInfo.bonusMove.bonusMoveDmg.mul;
            var dmgPlus = bonusInfo.bonusMove.bonusMoveDmg.add;

            ActivateNode(level, ref bonusInfo);
            
            var after = "";
            after = LevelUtilityV2.GetFlashDamage().ToString(GameConst.FloatFormat, CultureInfo.InvariantCulture);
            
            bonusInfo.bonusMove.bonusMoveDmg.mul = dmgMultiply;
            bonusInfo.bonusMove.bonusMoveDmg.add = dmgPlus;
        ...
        public INodeActivateLogicV2 Generate(string subType, List<string> listValue, bool mul)
        {
            ...
            {
                throw new Exception($"Invalid BonusMoveDmg value string: {listValue[0]}");
            }
        }
    }
}
