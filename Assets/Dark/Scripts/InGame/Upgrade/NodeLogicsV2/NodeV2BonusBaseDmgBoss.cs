...
namespace InGame.Upgrade.NodeLogicsV2
{
    ...
    public class NodeV2BonusBaseDmgBoss : INodeActivateLogicV2, INodeLogicGeneratorV2, INodeDynamicBonusValueV2
    {
        public float[] value;
        public bool isMul;
        ...
        public void OverrideBonusValue(int groupUnlockOrder)
        {
            if (MaxLevel == 1)
            {
                var dynamicValue = DynamicBonusValueConfig.Instance.GetBonus1Stage(NodeBonusTypeV2.BonusBaseDmgBoss, groupUnlockOrder);
                value = new[] { dynamicValue };
            }
            else
            {
                value = DynamicBonusValueConfig.Instance.GetBonus5Stage(NodeBonusTypeV2.BonusBaseDmgBoss, groupUnlockOrder).ToArray();
            }
        }
    }
}
