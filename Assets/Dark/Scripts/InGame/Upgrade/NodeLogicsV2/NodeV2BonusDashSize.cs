...
{
    [Serializable]
    [ConfigNodeLogicTypeV2(NodeBonusTypeV2.BonusDashSize)]
    public class NodeV2BonusDashSize : INodeActivateLogicV2, INodeLogicGeneratorV2
    {
        public float[] value;
        public bool isMul;
        ...
        public void ActivateNode(int level, ref UpgradeBonusInfoV2 bonusInfo)
        {
            ...
            if (isMul) bonusInfo.bonusMove.bonusDashSize.mul += value[level - 1];
            else bonusInfo.bonusMove.bonusDashSize.add += value[level - 1];
        }
        ...
        public (string, string) GetBeforeAfterValueTotalStat(int level, ref UpgradeBonusInfoV2 bonusInfo)
        {
            ...
            before = LevelUtilityV2.GetDashSize().ToString(GameConst.FloatFormat, CultureInfo.InvariantCulture);

            if (level > value.Length)
                return (before, before);
 
            var dashSizeMultiply = bonusInfo.bonusMove.bonusDashSize.mul;
            var dashSizePlus = bonusInfo.bonusMove.bonusDashSize.add;

            ActivateNode(level, ref bonusInfo);
            
            var after = "";
            after = LevelUtilityV2.GetDashSize().ToString(GameConst.FloatFormat, CultureInfo.InvariantCulture);
            
            bonusInfo.bonusMove.bonusDashSize.mul = dashSizeMultiply;
            bonusInfo.bonusMove.bonusDashSize.add = dashSizePlus;
        ...
        public INodeActivateLogicV2 Generate(string subType, List<string> listValue, bool mul)
        {
            ...
            {
                throw new Exception($"Invalid BonusDashSize value string: {listValue[0]}");
            }
        }
    }
}
