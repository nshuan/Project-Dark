...
namespace InGame.Upgrade.NodeLogicsV2
{
    ...
    public class NodeV2BonusPassiveCounterThunderRate : INodeActivateLogicV2, INodeLogicGeneratorV2
    {
        ...
        public (string, string) GetBeforeAfterValueTotalStat(int level, ref UpgradeBonusInfoV2 bonusInfo)
        {
            var before = "";
            if (isMul)
                before = (bonusInfo.bonusPassiveCounter.bonusThunderRate.mul * 100).ToString(GameConst.FloatFormat, CultureInfo.InvariantCulture) + "%";
            else
                before = (bonusInfo.bonusPassiveCounter.bonusThunderRate.add * 100).ToString(GameConst.FloatFormat, CultureInfo.InvariantCulture) + "%";
            ...
            var after = "";
            if (isMul)
                after = (bonusInfo.bonusPassiveCounter.bonusThunderRate.mul * 100).ToString(GameConst.FloatFormat, CultureInfo.InvariantCulture) + "%";
            else
                after = (bonusInfo.bonusPassiveCounter.bonusThunderRate.add * 100).ToString(GameConst.FloatFormat, CultureInfo.InvariantCulture) + "%";
