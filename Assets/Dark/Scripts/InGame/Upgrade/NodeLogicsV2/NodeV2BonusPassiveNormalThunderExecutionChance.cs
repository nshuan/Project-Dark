...
namespace InGame.Upgrade.NodeLogicsV2
{
    ...
    public class NodeV2BonusPassiveNormalThunderExecutionChance : INodeActivateLogicV2, INodeLogicGeneratorV2
    {
        ...
        {
            var before = "";
            before = (LevelUtilityV2.GetPassiveSize(PassiveTriggerType.DameByNormalAttack, PassiveType.Thunder) * 100).ToString(GameConst.FloatFormat, CultureInfo.InvariantCulture) + "%";
            ...
            var after = "";
            after = (LevelUtilityV2.GetPassiveSize(PassiveTriggerType.DameByNormalAttack, PassiveType.Thunder) * 100).ToString(GameConst.FloatFormat, CultureInfo.InvariantCulture) + "%";
