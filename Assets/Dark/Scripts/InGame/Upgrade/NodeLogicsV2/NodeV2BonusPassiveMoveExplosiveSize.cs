...
namespace InGame.Upgrade.NodeLogicsV2
{
    ...
    public class NodeV2BonusPassiveMoveExplosiveSize : INodeActivateLogicV2, INodeLogicGeneratorV2
    {
        ...
        public (string, string) GetBeforeAfterValueTotalStat(int level, ref UpgradeBonusInfoV2 bonusInfo)
        {
            var before = "";
            if (isMul)
                before = (bonusInfo.bonusPassiveMove.bonusExplosiveSize.mul * 100).ToString(GameConst.FloatFormat, CultureInfo.InvariantCulture) + "%";
            else
                before = bonusInfo.bonusPassiveMove.bonusExplosiveSize.add.ToString(GameConst.FloatFormat, CultureInfo.InvariantCulture);
            ...
            var after = "";
            if (isMul)
                after = (bonusInfo.bonusPassiveMove.bonusExplosiveSize.mul * 100).ToString(GameConst.FloatFormat, CultureInfo.InvariantCulture) + "%";
            else
                after = bonusInfo.bonusPassiveMove.bonusExplosiveSize.add.ToString(GameConst.FloatFormat, CultureInfo.InvariantCulture);
